...
//        asn_sequence_add(&BSMCACC->caccData, my_caccdata);
	...
	while (1) {
		...
		if(debug) {
			...
			strcpy(&comm_pkt.object_id[0], "barf");
		}

		if(verbose)
printf("Got to 2 msgID %hhu seq_no %d global_time %f userbit1 %d userbit2 %d userbit3 %d userbit4 %d my_pip %d maneuver_id %d fault_mode %d maneuver_des_1 %d pltn_size %d acc_traj %f veh_traj %f range %f rate %f velocity %f acceleration %f\n", 
		(unsigned char)BSMCACC->msgID,
		...
		if(comm_pkt.my_pip == 2) {
			db_clt_read(pclt, DB_COMM_LEAD_TRK_VAR, sizeof(comm_pkt1), &comm_pkt1);
			db_clt_read(pclt, DB_COMM_THIRD_TRK_VAR, sizeof(comm_pkt3), &comm_pkt3);
			if( (comm_pkt1.my_pip == 1) && (comm_pkt3.my_pip == 3) ){
				memset(BSMCACC, 0, sizeof(BSMCACC_t));
				memset(BSMCACC_buf, 0, BSMCACCSIZE);

				ret = vehcomm2BSM(BSMCACC, &comm_pkt1);
				erv = der_encode_to_buffer(&asn_DEF_BSMCACC, BSMCACC, BSMCACC_buf, sizeof(BSMCACC_t));
				...
				if(erv.encoded < 0) {
					...
					exit(EXIT_FAILURE);
		                ...
		                bytes_sent = sendto(sd, &BSMCACC_buf, erv.encoded, 0, (struct sockaddr *) &dst_addr, sizeof(dst_addr));
	                	if (bytes_sent < 0) {
	                	        perror("veh_snd: UDP sendto ");
	                	        printf("port %d addr 0x%08x\n",
	                	                ntohs(dst_addr.sin_port),
				...
				ret = vehcomm2BSM(BSMCACC, &comm_pkt3);
				erv = der_encode_to_buffer(&asn_DEF_BSMCACC, BSMCACC, BSMCACC_buf, sizeof(BSMCACC_t));
