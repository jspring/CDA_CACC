...
//	cacc_data->desTrq = (long)(comm_pkt->engine_torque * BSM_FLOAT_MULT);
...
//		comm_pkt->engine_torque = (long)(cacc_data->desTrq / BSM_FLOAT_MULT);
...
//printf("VEH_LIB: vehicle %d brake %d\n", comm_pkt->object_id, comm_pkt->brake); // From long_ctl or trk_comm_mgr)
