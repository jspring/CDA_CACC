...
int can_arm(int fd, int channel_id)
{
        ...
        SIGEV_PULSE_INIT(&phdl->pulse_event, phdl->connect_id, 1, 
...
int can_open(char *filename, int flags)
{
	...
	char devname[10] = "CANDRV1";
        ...
        switch (can_channel) {
                ...
                case 3: 
                case 4: 
                        printf("Valid channel number %d. ", can_channel);
                        if(can_channel > 2) {
                        	can_channel -= 2;
							strcpy(devname, "CANDRV2");
                        }
                        printf(" Mapping to channel number %d, device name %s \n", can_channel, devname);
                        break;
                default:
        ...
        if ( (err = ConnectDriver (can_channel, devname, &phdl->hdl)) < 0) {
        	if(err == ERR_CHANNEL)
                printf ("can_open: can't connect the CAN driver: Invalid channel number\n");
        	if(err == ERR_NO_DRV)
                printf ("can_open: can't connect the CAN driver: CAN driver not started or not reachable\n");
                return (0);
        }  
