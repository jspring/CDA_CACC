...
int main(int argc, char *argv[])
{
	...
	if ((pclt = db_list_init(argv[0], hostname, domain, xport, 
			db_vars_list_neweagle_steering, NUM_DB_VARS, NULL, 0)) == NULL) {
		printf("Database initialization error in %s.\n", argv[0]);
		exit(EXIT_FAILURE);
	}
	for (i = 0; i < NUM_DB_VARS; i++){
		db_clt_write(pclt,
		db_vars_list_neweagle_steering[i].id,
		db_vars_list_neweagle_steering[i].size,
		zero_array);
	}
	...
	if(( exitsig = setjmp(exit_env)) != 0) {
		db_list_done(pclt, db_vars_list_neweagle_steering, NUM_DB_VARS, NULL, 0);
		exit(EXIT_SUCCESS);
	} else
