...
#define NUM_LEDDAR_VARS	(sizeof(leddar_vars)/sizeof(db_id_t))
...
DisplayDetections( LtBool aAlt )
{
    ...
    while(1)
    {
		...
		if(verbose) {
            ...
            printf("DetectionCount: %d\n", lAcquisition.mDetectionCount);
...
//			printf("Using database! db_num %d\n", db_num);
        ...
        else
        {
        	...
        	if(create_db_vars)
                db_list_done(pclt, leddar_vars, NUM_LEDDAR_VARS, NULL, 0);
        	else
...
main( int argc, char *argv[] )
{
 ...
 //                       	printf("ser1\n");
                ...
                case 'v':
                        ...
                        break;
                default:
               		printf("Usage: %s -p <port, def. /dev/ser1> -a <sensor address, def. 1> -d (use database) -c (create database variables) -v (verbose)\n", argv[0]);
               		exit(EXIT_FAILURE);
                        break;
                }
        }
	...
	if(use_db) {
		...
		if(create_db_vars) {
			if ( (pclt = db_list_init(argv[0], hostname, domain, xport,
				leddar_vars, NUM_LEDDAR_VARS, NULL, 0)) == NULL) {
					exit(EXIT_FAILURE);
			}
		}
		else {
			if ( (pclt = db_list_init(argv[0], hostname, domain, xport,
					leddar_vars, 0, NULL, 0))
				== NULL) {
					exit(EXIT_FAILURE);
			}
		}
